...
const ap=path.join(ROOT,'experiments','026','hidden','ASSERTIONS.json');
...
if(!fs.existsSync(rp)||!fs.existsSync(ap)){
  ...
  fs.writeFileSync(sp,JSON.stringify({experiment:'026',disposition:'INFRASTRUCTURE_FAILURE'},null,2)+'\n');
  process.exit(1);
}

const report=JSON.parse(fs.readFileSync(rp,'utf8'));
const assertions=JSON.parse(fs.readFileSync(ap,'utf8'));
...
for(const id of assertions.required_case_ids){
  ...
  if(!actual){
    ...
    if(typeof actual.reason!=='string'||actual.reason.trim()==='') mismatches.push({field:'reason',expected:'nonempty',actual:actual.reason??null});
...
const guards={
  ...
  module_assessment_consistent:!allPass||report?.module_assessment?.dts_0_1==='SUPPORTED'
};
...
const score={
  experiment:'026',
  disposition,
  dts_0_1:{pass:allPass,failed_cases:results.filter(r=>!r.pass).map(r=>r.case_id)},
  guards,
  duplicates,
