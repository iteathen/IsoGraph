...
const OUT=path.join(ROOT,'out','exp017');
...
if(!fs.existsSync(reportPath)||!fs.existsSync(assertionsPath)){
  ...
  fs.writeFileSync(scorePath,JSON.stringify({experiment:'017',disposition:'INFRASTRUCTURE_FAILURE',error:'missing report or assertions'},null,2)+'\n');
  process.exit(1);
}
...
const guards={
  ...
  module_assessment_consistent:!allPass||report?.module_assessment?.core_0_18==='SUPPORTED'
};
const disposition=allPass&&Object.values(guards).every(Boolean)?'QUALIFIES':'DOES_NOT_QUALIFY';
const score={experiment:'017',disposition,core_0_18:{pass:allPass,failed_cases:results.filter(x=>!x.pass).map(x=>x.case_id)},guards,duplicates,unexpected_case_ids:unexpected,case_results:results};
fs.mkdirSync(OUT,{recursive:true});
