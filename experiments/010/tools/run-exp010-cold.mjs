...
const inputs=[
  ...
  'experiments/010/DP_CASES.md'
];
const promptPath='experiments/010/COLD_PROMPT.md';
const forbidden=['hidden/','ASSERTIONS','score-exp010','AUTHORING_AUDIT','QUALIFICATION_REVIEW','experiments/007','experiments/008'];
function frozen(p){return execFileSync('git',['show',`${SHA}:${p}`],{encoding:'utf8',maxBuffer:128*1024*1024});}
function sha256(v){return crypto.createHash('sha256').update(v).digest('hex');}

fs.mkdirSync('out/exp010',{recursive:true});
for(const p of [...inputs,promptPath]){
  if(forbidden.some(x=>p.includes(x))) throw new Error('Forbidden cold input: '+p);
  frozen(p);
}
const cases=frozen('experiments/010/DP_CASES.md');
for(let i=1;i<=8;i++){const id=`D${String(i).padStart(2,'0')}`;if(!cases.includes(`## ${id} `)) throw new Error('Missing '+id);}

const manifest=[], chunks=[`ISOGRAPH EXPERIMENT 010 — CUMULATIVE DISCOVERY PROTOCOL QUALIFICATION\nRun: ${RUN}\nFrozen SHA: ${SHA}\nModel: ${MODEL}\n\nISOLATION: only delimited files are available; no prior experiment reports, hidden oracle, scorer, repository map, agent guidance, or browsing.\n`];
...
fs.writeFileSync(`out/exp010/${RUN}_PACKET.txt`,packet);
fs.writeFileSync(`out/exp010/${RUN}_INPUT_MANIFEST.json`,JSON.stringify(manifest,null,2)+'\n');
if(DRY){
  const d={run:RUN,dry_run:true,qualification_sha:SHA,model:MODEL,packet_sha256:packetHash,input_manifest:manifest};
  fs.writeFileSync('out/exp010/DRY_RUN.json',JSON.stringify(d,null,2)+'\n'); console.log(JSON.stringify(d)); process.exit(0);
}
...
fs.writeFileSync(`out/exp010/${RUN}_API_RESPONSE.json`,responseText);
const meta={experiment:'010',run:RUN,model:MODEL,qualification_sha:SHA,workflow_run_id:process.env.GITHUB_RUN_ID||null,workflow_attempt:process.env.GITHUB_RUN_ATTEMPT||null,input_manifest:manifest,packet_sha256:packetHash,api_attempts:attempts,http_status:status};
if(!(status>=200&&status<300)){fs.writeFileSync('out/exp010/METADATA.json',JSON.stringify({...meta,semantic_status:'PROVIDER_FAILURE'},null,2)+'\n');throw new Error(`Gemini HTTP ${status}`);}
const data=JSON.parse(responseText);
const raw=(data.candidates?.[0]?.content?.parts||[]).filter(p=>!p.thought).map(p=>p.text||'').join('').trim();
fs.writeFileSync('out/exp010/COLD_REPORT_RAW.txt',raw+'\n');
let jt=raw.replace(/^\`\`\`(?:json)?\s*/i,'').replace(/\s*\`\`\`$/,'');const a=jt.indexOf('{'),b=jt.lastIndexOf('}');if(a>=0&&b>=a)jt=jt.slice(a,b+1);
let parsed;try{parsed=JSON.parse(jt);}catch(e){fs.writeFileSync('out/exp010/METADATA.json',JSON.stringify({...meta,semantic_status:'MALFORMED_OUTPUT',report_sha256:sha256(raw)},null,2)+'\n');throw e;}
fs.writeFileSync('out/exp010/PARSED_REPORT.json',JSON.stringify(parsed,null,2)+'\n');
fs.writeFileSync('out/exp010/METADATA.json',JSON.stringify({...meta,semantic_status:'FROZEN',report_sha256:sha256(raw),finish_reason:data.candidates?.[0]?.finishReason??null,usage:data.usageMetadata??null},null,2)+'\n');
