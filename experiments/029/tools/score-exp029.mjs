...
const assertionsPath=path.join(ROOT,'experiments','029','hidden','ASSERTIONS.json');
...
if(!fs.existsSync(reportPath)||!fs.existsSync(assertionsPath)){
  ...
  fs.writeFileSync(scorePath,JSON.stringify({experiment:'029',disposition:'INFRASTRUCTURE_FAILURE'},null,2)+'\n');
  process.exit(1);
}
...
const guards={
  ...
  module_assessment_consistent:!allPass||report?.module_assessment?.core_0_19==='SUPPORTED'
};
...
const score={
  ...
  core_0_19:{
    ...
    implicit_assertion_cases:results.filter(result=>Number(result.case_id.slice(1))<=18).map(result=>({case_id:result.case_id,pass:result.pass})),
    exact_rendering_cases:results.filter(result=>Number(result.case_id.slice(1))>=19).map(result=>({case_id:result.case_id,pass:result.pass}))
  },
  guards,
  duplicates,
