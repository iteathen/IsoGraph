...
const assertionsPath=path.join(ROOT,'experiments','028','hidden','ASSERTIONS.json');
...
if(!fs.existsSync(reportPath)||!fs.existsSync(assertionsPath)){
  ...
  fs.writeFileSync(scorePath,JSON.stringify({experiment:'028',disposition:'INFRASTRUCTURE_FAILURE'},null,2)+'\n');
  process.exit(1);
}
...
const guards={
  ...
  module_assessment_consistent:!allPass||report?.module_assessment?.dp_0_7_delta==='SUPPORTED'
};
