...
const inputs=[
  'experiments/025/FOCUSED_AUTHORITY.md',
  'experiments/025/QU_PROJECTION_WITNESS.json',
  'experiments/021/TRANSITIONS.json',
  'experiments/021/VIEWS.json'
];
const promptPath='experiments/025/COLD_PROMPT.md';
const forbidden=['hidden/','CONTROLS','score-exp025','AUTHORING_AUDIT','RESULTS','experiments/025/evidence','AGENTS.md','STATUS.md'];
...
fs.mkdirSync('out/exp025',{recursive:true});
for(const path of [...inputs,promptPath]){
  if(forbidden.some(x=>path.includes(x))) throw new Error('forbidden cold input '+path);
  frozen(path);
}

const witness=JSON.parse(frozen('experiments/025/QU_PROJECTION_WITNESS.json'));
if(witness.qu_region_ownership?.length!==4) throw new Error('expected four QU ownership records');

const manifest=[];
const chunks=[`ISOGRAPH EXPERIMENT 025 — R07 QU PROJECTION AUDIT
Run: ${RUN}
Frozen SHA: ${SHA}
Model: ${MODEL}

ISOLATION: use only the delimited packet. Hidden controls/scorer, prior Experiment 025 evidence, Experiment 024 hidden expectation, and browsing are unavailable.
`];
...
fs.writeFileSync('out/exp025/'+RUN+'_PACKET.txt',packet);
fs.writeFileSync('out/exp025/'+RUN+'_INPUT_MANIFEST.json',JSON.stringify(manifest,null,2)+'\n');

if(DRY){
  const dry={run:RUN,dry_run:true,source_sha:SHA,model:MODEL,packet_sha256:packetHash,input_manifest:manifest};
  fs.writeFileSync('out/exp025/DRY_RUN.json',JSON.stringify(dry,null,2)+'\n');
...
const metadata={
  experiment:'025',run:RUN,model:MODEL,source_sha:SHA,
...
if(!(status>=200&&status<300)){
  fs.writeFileSync('out/exp025/METADATA.json',JSON.stringify({...metadata,semantic_status:'PROVIDER_FAILURE'},null,2)+'\n');
...
fs.writeFileSync('out/exp025/COLD_REPORT_RAW.txt',raw+'\n');
...
try{parsed=JSON.parse(jt);}
  ...
  fs.writeFileSync('out/exp025/METADATA.json',JSON.stringify({...metadata,semantic_status:'MALFORMED_OUTPUT',report_sha256:sha256(raw)},null,2)+'\n');
  throw error;
}

fs.writeFileSync('out/exp025/PARSED_REPORT.json',JSON.stringify(parsed,null,2)+'\n');
fs.writeFileSync('out/exp025/METADATA.json',JSON.stringify({...metadata,semantic_status:'FROZEN',report_sha256:sha256(raw),finish_reason:data.candidates?.[0]?.finishReason??null,usage:data.usageMetadata??null},null,2)+'\n');
