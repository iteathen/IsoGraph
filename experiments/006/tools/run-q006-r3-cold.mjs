...
const promptPath = 'experiments/006/COLD_Q006_R3_PROMPT.md';
const forbidden = ['Q006_ASSERTIONS','DP_HIDDEN_ORACLE','SCORE','R2_REVIEW','REPRODUCTION'];
...
fs.mkdirSync('out/q006r3', { recursive: true });
...
fs.writeFileSync(`out/q006r3/${RUN}_PACKET.txt`,packet);
fs.writeFileSync(`out/q006r3/${RUN}_INPUT_MANIFEST.json`,JSON.stringify(manifest,null,2)+'\n');
...
fs.writeFileSync(`out/q006r3/${RUN}_API_RESPONSE.json`,responseText);
...
if (!(status>=200&&status<300)) {
  fs.writeFileSync(`out/q006r3/${RUN}_METADATA.json`,JSON.stringify({...meta,semantic_status:'PROVIDER_FAILURE'},null,2)+'\n');
...
fs.writeFileSync(`out/q006r3/${RUN}_COLD_REPORT_RAW.txt`,raw+'\n');
...
try { parsed=JSON.parse(jt); } catch(e) {
  fs.writeFileSync(`out/q006r3/${RUN}_METADATA.json`,JSON.stringify({...meta,semantic_status:'MALFORMED_OUTPUT',report_sha256:sha256(raw),finish_reason:data.candidates?.[0]?.finishReason??null,usage:data.usageMetadata??null},null,2)+'\n');
  throw e;
}
fs.writeFileSync(`out/q006r3/${RUN}_PARSED_REPORT.json`,JSON.stringify(parsed,null,2)+'\n');
fs.writeFileSync(`out/q006r3/${RUN}_METADATA.json`,JSON.stringify({...meta,semantic_status:'FROZEN',report_sha256:sha256(raw),finish_reason:data.candidates?.[0]?.finishReason??null,usage:data.usageMetadata??null},null,2)+'\n');
