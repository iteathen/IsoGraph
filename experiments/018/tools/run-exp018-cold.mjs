...
const inputs=[
  ...
  'experiments/018/BASELINE_AUTHORITY.md',
  'experiments/018/DP_0_6_CASES.md'
];
const promptPath='experiments/018/COLD_PROMPT.md';
const forbidden=['hidden/','ASSERTIONS','score-exp018','AUTHORING_AUDIT','QUALIFICATION_REVIEW','experiments/018/evidence','AGENTS.md','README.md','STATUS.md'];
...
const cases=frozen('experiments/018/DP_0_6_CASES.md');
for(let i=1;i<=14;i++){
  const id='D'+String(i).padStart(2,'0');
  if(!cases.includes('## '+id+' '))throw new Error('Missing '+id);
}
const manifest=[];
const chunks=[`ISOGRAPH EXPERIMENT 018 — DP 0.5/0.6 SUCCESSOR QUALIFICATION
Run: ${RUN}
Frozen SHA: ${SHA}
Model: ${MODEL}

ISOLATION: use only delimited files. DP 0.1-0.4 are already-qualified baseline authority and are not being re-proved in this focused successor campaign. Prior outputs, hidden assertions/scorer, author audits, repository map, agent guidance, and browsing are unavailable.
`];
...
const request={
  ...
  generationConfig:{candidateCount:1,maxOutputTokens:32768,temperature:0.1,thinkingConfig:{thinkingLevel:'HIGH'}}
};
...
if(!(status>=200&&status<300)){
  fs.writeFileSync('out/exp018/METADATA.json',JSON.stringify({...meta,semantic_status:'PROVIDER_FAILURE'},null,2)+'\n');
...
const a=jt.indexOf('{'),z=jt.lastIndexOf('}');
if(a>=0&&z>=a)jt=jt.slice(a,z+1);
let parsed;
try{parsed=JSON.parse(jt);}
catch(e){
  fs.writeFileSync('out/exp018/METADATA.json',JSON.stringify({...meta,semantic_status:'MALFORMED_OUTPUT',report_sha256:sha256(raw)},null,2)+'\n');
  throw e;
}
