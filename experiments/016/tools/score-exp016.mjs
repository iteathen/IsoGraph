...
const OUT=path.join(ROOT,'out','exp016');
const reportPath=path.join(OUT,'PARSED_REPORT.json');
const assertionsPath=path.join(ROOT,'experiments','016','hidden','ASSERTIONS.json');
...
if(!fs.existsSync(reportPath)||!fs.existsSync(assertionsPath)){
  ...
  fs.writeFileSync(scorePath,JSON.stringify({experiment:'016',disposition:'INFRASTRUCTURE_FAILURE',error:'missing report or assertions'},null,2)+'\n');
  process.exit(1);
}
...
const guards={
  ...
  module_assessment_consistent:!allPass||report?.module_assessment?.nei_0_4==='SUPPORTED'
};
const disposition=allPass&&Object.values(guards).every(Boolean)?'QUALIFIES':'DOES_NOT_QUALIFY';
const score={experiment:'016',disposition,nei_0_4:{pass:allPass,failed_cases:results.filter(x=>!x.pass).map(x=>x.case_id)},guards,duplicates,unexpected_case_ids:unexpected,case_results:results};
fs.mkdirSync(OUT,{recursive:true});
