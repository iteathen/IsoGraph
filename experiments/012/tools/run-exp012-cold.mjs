...
fs.writeFileSync(`out/exp012/${RUN}_PACKET.txt`,packet);
fs.writeFileSync(`out/exp012/${RUN}_INPUT_MANIFEST.json`,JSON.stringify(manifest,null,2)+'\n');

if(DRY){
  const d={run:RUN,dry_run:true,qualification_sha:SHA,model:MODEL,packet_sha256:packetHash,input_manifest:manifest};
  fs.writeFileSync('out/exp012/DRY_RUN.json',JSON.stringify(d,null,2)+'\n');
...
fs.writeFileSync(`out/exp012/${RUN}_API_RESPONSE.json`,responseText);
const meta={experiment:'012',run:RUN,model:MODEL,qualification_sha:SHA,workflow_run_id:process.env.GITHUB_RUN_ID||null,workflow_attempt:process.env.GITHUB_RUN_ATTEMPT||null,input_manifest:manifest,packet_sha256:packetHash,api_attempts:attempts,http_status:status};
if(!(status>=200&&status<300)){
  fs.writeFileSync('out/exp012/METADATA.json',JSON.stringify({...meta,semantic_status:'PROVIDER_FAILURE'},null,2)+'\n');
...
fs.writeFileSync('out/exp012/COLD_REPORT_RAW.txt',raw+'\n');
...
try{parsed=JSON.parse(jt);}
  ...
  fs.writeFileSync('out/exp012/METADATA.json',JSON.stringify({...meta,semantic_status:'MALFORMED_OUTPUT',report_sha256:sha256(raw)},null,2)+'\n');
  throw e;
}
fs.writeFileSync('out/exp012/PARSED_REPORT.json',JSON.stringify(parsed,null,2)+'\n');
fs.writeFileSync('out/exp012/METADATA.json',JSON.stringify({...meta,semantic_status:'FROZEN',report_sha256:sha256(raw),finish_reason:data.candidates?.[0]?.finishReason??null,usage:data.usageMetadata??null},null,2)+'\n');
