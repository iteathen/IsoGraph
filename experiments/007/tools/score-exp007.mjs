...
const OUT=process.env.EXP007_OUT || 'out/exp007';
const reportPath=`${OUT}/EXP007_PARSED_REPORT.json`;
const scorePath=`${OUT}/EXP007_SCORE.json`;
const cfg=JSON.parse(fs.readFileSync('experiments/007/ASSERTIONS.json','utf8'));
...
if(!fs.existsSync(reportPath)){
  const score={run:'EXP007',execution_status:'NO_PARSED_REPORT',disposition:'DOES_NOT_QUALIFY'};
...
const rejected=new Set((r.rejected_overclaims||[]).map(norm));
...
const score={
  ...
  missing_rejected_overclaims:missingReject,
  guards
};
fs.writeFileSync(scorePath,JSON.stringify(score,null,2)+'\n');
console.log(JSON.stringify({run:'EXP007',disposition,matched,deep,guards}));
