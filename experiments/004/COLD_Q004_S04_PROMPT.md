...
^6405  pairwise common-core body in A-side identities
^6406  A-side pair residual
^6407  B-side pair residual
^6408  lossy-discard summary; identifies omitted relation roles but does not retain omitted incidences
^6409  pairwise common-core A-to-B identity mapping
```
...
3. Audit pair record `3000`: reproduce common core, A-to-B mapping, and each pair residual, then show whether `1200` and `2200` can each be reconstructed at the comparison stage from that pairwise accounting.
