...
const inputs=[
  ...
  'experiments/021/BASELINE_AUTHORITY.md'
];
const promptPath='experiments/021/COLD_PROMPT.md';
const forbidden=[
  'hidden/','ASSERTIONS','score-exp021','AUTHORING_AUDIT','RESULTS',
  'experiments/021/evidence','AGENTS.md','README.md','STATUS.md'
];
...
fs.mkdirSync('out/exp021',{recursive:true});
...
if(corpus.transitions?.length!==7) throw new Error('Expected seven transition records');
if(views.views?.length!==4) throw new Error('Expected four comparison views');

const manifest=[];
const chunks=[`ISOGRAPH EXPERIMENT 021 — FIRST DTS TI DISCOVERY CAMPAIGN
Run: ${RUN}
Frozen SHA: ${SHA}
Model: ${MODEL}

ISOLATION: use only the delimited files. Hidden assertions/scorer, prior outputs/evidence, author audits, repository routing text, expected pairings, and browsing are unavailable.
`];
...
fs.writeFileSync('out/exp021/'+RUN+'_PACKET.txt',packet);
fs.writeFileSync('out/exp021/'+RUN+'_INPUT_MANIFEST.json',JSON.stringify(manifest,null,2)+'\n');

if(DRY){
  const dry={run:RUN,dry_run:true,qualification_sha:SHA,model:MODEL,packet_sha256:packetHash,input_manifest:manifest};
  fs.writeFileSync('out/exp021/DRY_RUN.json',JSON.stringify(dry,null,2)+'\n');
...
if(!(status>=200&&status<300)){
  fs.writeFileSync('out/exp021/METADATA.json',JSON.stringify({...metadata,semantic_status:'PROVIDER_FAILURE'},null,2)+'\n');
...
fs.writeFileSync('out/exp021/COLD_REPORT_RAW.txt',raw+'\n');
...
try{
  ...
  fs.writeFileSync('out/exp021/METADATA.json',JSON.stringify({...metadata,semantic_status:'MALFORMED_OUTPUT',report_sha256:sha256(raw)},null,2)+'\n');
  throw error;
}

fs.writeFileSync('out/exp021/PARSED_REPORT.json',JSON.stringify(parsed,null,2)+'\n');
fs.writeFileSync('out/exp021/METADATA.json',JSON.stringify({
  ...metadata,
