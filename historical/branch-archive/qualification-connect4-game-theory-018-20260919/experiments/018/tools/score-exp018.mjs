...
const OUT=path.join(ROOT,'out','exp018');
const reportPath=path.join(OUT,'PARSED_REPORT.json');
const assertionsPath=path.join(ROOT,'experiments','018','hidden','ASSERTIONS.json');
const scorePath=path.join(OUT,'SCORE.json');
function deepEqual(a,b){return JSON.stringify(a)===JSON.stringify(b);}

if(!fs.existsSync(reportPath)||!fs.existsSync(assertionsPath)){
  fs.mkdirSync(OUT,{recursive:true});
  fs.writeFileSync(scorePath,JSON.stringify({experiment:'018',disposition:'INFRASTRUCTURE_FAILURE',error:'missing report or assertions'},null,2)+'\n');
  process.exit(1);
}
...
for(const id of assertions.required_case_ids){
  ...
  if(!actual)mismatches.push({field:'case_id',expected:id,actual:null});
    ...
    const expectedKeys=Object.keys(spec.answers).sort(), actualKeys=Object.keys(actual?.answers??{}).sort();
...
const guards={
 ...
 assessment_consistent:!allPass||report?.package_assessment?.connect4_game_theory_1_2==='SUPPORTED'
};
const disposition=allPass&&Object.values(guards).every(Boolean)?'QUALIFIES':'DOES_NOT_QUALIFY';
const score={experiment:'018',disposition,connect4_game_theory_1_2:{pass:allPass,failed_cases:results.filter(x=>!x.pass).map(x=>x.case_id)},guards,duplicates,unexpected_case_ids:unexpected,case_results:results};
fs.mkdirSync(OUT,{recursive:true});
